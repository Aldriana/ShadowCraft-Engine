...
class RogueDamageCalculator(DamageCalculator):
    ...
    def talents_modifiers(opportunity=False, coup_de_grace=False,
                          ...
                          improved_sinister_strike=False,
                          improved_ambush=False, potent_poisons=False,
                          assassins_resolve=True):
        ...
        if executioner and self.talents.is_subtlety_rogue():
            base_modifier += .2 + .025 * self.stats.get_mastery_from_rating()
        if aggression:
        ...
        if potent_poisons and self.talents.is_assassination_rogue():
            base_modifier *= 1.2 + .025 * self.stats.get_mastery_from_rating()
    ...
    def ambush_damage(self, ap):
        ...
        if self.stats.mh.type == 'dagger':
            damage = 2.7493 * weapon_damage + 367 * 2.75
        else:
            damage = 1.9 * weapon_damage + 367 * 1.9
    ...
    def envenom_damage(self, ap, cp):
        ...
        multiplier = talents_modifiers(coup_de_grace=True, executioner=True,
                                       assassins_resolve=False potent_poisons=True)
