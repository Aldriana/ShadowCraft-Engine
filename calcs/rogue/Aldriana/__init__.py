...
class AldrianasRogueDamageCalculator(RogueDamageCalculator):
    ...
    def assassination_dps_breakdown_mutilate(self):
        ...
        while True:
            attacks_per_second, crit_rates = self.assassination_attack_counts_mutilate(current_stats)
            ...
            for proc in active_procs:
                ...
                if proc.procs_off_strikes():
                    triggers_per_second += 2 * attacks_per_second['mutilate'] + sum(attacks_per_second['envenom'])
                if proc.procs_off_harmful_spells():
                    triggers_per_second += attacks_per_second['instant_poison'] + attacks_per_second['venomous_wounds']
