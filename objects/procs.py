class Proc(object):
    ...
    # I know the case in question (rupture) but I've never heard of a proc specific to this situation
    # assuming this is the same as strikes for now
...
class PPMProc(Proc):
    ...
    def proc_rate(self,speed):
        return self.proc_chance / 60 * speed / 100

    def is_ppm(self):
        return True

class ProcsList(object):
    #Format is (stat, value per stack, duration, proc rate, trigger, ICD, max stacks)
    ...
    allowed_procs = {
        ...
        'hurricane':                                PPMProc('haste', 450, 12, 1, 'all_spells_and_attacks', 0, 3),
        'key_to_the_endless_chamber':               Proc('agi', 1290, 15, .1, 'all_attacks', 75, 1),
        'landslide':                                PPMProc('ap', 1000, 12, None, 'all_attacks', None, None), #Guessing max_stacks is 2, will wait for confirmation
        ...
        'rogue_t11_4pc':                            Proc('weird_proc', 1, 15, .01, 'auto_attacks', None, 1), #We know for sure it's 1?
    ...
    #This is a reference - it is not used directly
